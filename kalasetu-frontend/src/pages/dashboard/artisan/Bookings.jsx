...
import { Badge, EmptyState, LoadingState, Alert } from '../../../components/ui';
...
const Bookings = () => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 font-display">Bookings & Schedule</h1>
      ...
      {filteredBookings.length > 0 ? (
        <div className="space-y-4">
          {filteredBookings.map(booking => (
            <div key={booking._id} className="bg-white border border-gray-200 rounded-card p-6 shadow-card card-hover">
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-900">{booking.user?.fullName || 'N/A'}</span>
                    </div>
              ...
              {booking.status === 'pending' && (
                <div className="flex gap-2 flex-wrap pt-4 border-t border-gray-100">
                  <button
                    ...
                    className="btn-press px-4 py-2 text-sm bg-success-600 text-white rounded-button hover:bg-success-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    disabled={actionLoading === booking._id}
                    className="btn-press px-4 py-2 text-sm border border-error-300 text-error-600 rounded-button hover:bg-error-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
              ...
              {booking.status === 'confirmed' && (
                <div className="flex gap-2 flex-wrap pt-4 border-t border-gray-100">
                  <button
                    ...
                    disabled={actionLoading === booking._id}
                    className="btn-press px-4 py-2 text-sm bg-brand-500 text-white rounded-button hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {actionLoading === booking._id ? 'Processing...' : 'Mark as Completed'}
                  </button>
                  <p className="self-center text-sm text-gray-500">Booking confirmed - ready to start work</p>
                </div>
              )}
            </div>
          ))}
