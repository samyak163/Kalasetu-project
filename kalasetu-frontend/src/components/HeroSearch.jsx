...
const HeroSearch = () => {
  ...
  const quickCategories = useMemo(() => {
    const list = (categories || []).slice(0, 5).map((category) => category.name);
    if (list.length === 0) {
      return ['Handicrafts', 'Home Services', 'Food & Catering', 'Clothing & Tailoring', 'Wellness & Beauty'];
    }
    return list;
  }, [categories]);
      ...
      <div className="relative container mx-auto text-center z-10 max-w-4xl">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {(loading && categories.length === 0
            ...
            <button
              key={typeof category === 'string' ? category : `category-skeleton-${index}`}
              onClick={() => typeof category === 'string' && navigate(`/search?category=${encodeURIComponent(category)}`)}
              ...
              disabled={typeof category !== 'string'}
            >
              {typeof category === 'string' ? category : 'Loading...'}
            </button>
          ))}
        </div>
      </div>
