...
import { useChatUnread } from '../context/ChatContext.jsx';
...
const Header = () => {
  ...
  const { notifications, unreadCount, refresh: refreshNotifications, markRead } = useNotifications();
  ...
  return (
    <header className="!bg-white border-b border-gray-200 sticky top-0 z-50">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 !text-black" aria-label="Top">
        <div className="w-full py-4 flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center space-x-4 flex-shrink-0">
            ...
            {auth.user && (
              <button onClick={() => { refreshNotifications(); setShowNotifications(true); }} aria-label="Notifications" className="relative text-gray-700 hover:text-brand-500">
                ...
                {unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] leading-3 px-1.5 py-0.5 rounded-full">{unreadCount > 9 ? '9+' : unreadCount}</span>}
              </button>
            )}
            ...
            {auth.user && (
              <>
                ...
                >
                  ...
                  {chatUnread > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] leading-3 px-1.5 py-0.5 rounded-full">{chatUnread > 9 ? '9+' : chatUnread}</span>}
                </Link>
                <Link
      ...
      <NotificationPanel isOpen={showNotifications} onClose={() => setShowNotifications(false)} notifications={notifications} unreadCount={unreadCount} onMarkRead={markRead} />
      <ProfileModal />
    </header>
  );
};
